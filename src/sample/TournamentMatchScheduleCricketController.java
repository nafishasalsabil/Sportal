...
public class TournamentMatchScheduleCricketController implements Initializable {
    ...
    void see_match_standings_pressed(ActionEvent event) throws IOException {
        TournamentStandingCricketController tournamentMatchScheduleController=new TournamentStandingCricketController();
        tournamentMatchScheduleController.tname(tournament_name1);

        AnchorPane pane = FXMLLoader.load(getClass().getClassLoader().getResource("sample/Tournament_Standing_Cricket.fxml"));
...
//            data.add(new MatchScheduleClass(team_nameA,team_nameB,match_date,match_time,Integer.toString(venue_id),Integer.toString(facilitator_id),Integer.toString(referee_id)));
            ...
            System.out.println(tournament_name1+"))))))))");
            PreparedStatement preparedStatement_tournament = connection.prepareStatement("select tournament_id FROM tournament where tournament_name=? and sport_id=2;");
...
//                tournament_id1=Integer.parseInt(resultSet1.getString("tournament_id"));
    ...
    public void add_match_pressed(ActionEvent actionEvent) throws IOException {
        AddCricketMatchDialogBoxController addFootballMatchDialogBoxController = new AddCricketMatchDialogBoxController();
        ...
        AnchorPane pane = FXMLLoader.load(getClass().getClassLoader().getResource("sample/AddCricketMatchDialogBox.fxml"));
...
//        tournament_name1=tournament_name_pass;
    ...
    public void match_clicked(MouseEvent mouseEvent) throws IOException {
        ...
        if(match_finished==true){
            ...
            CompletedMatchStatsCricketController completedMatchStatsController = new CompletedMatchStatsCricketController();
            completedMatchStatsController.pass_match_id(match_id_list.get( schedule_listview.getSelectionModel().getSelectedIndex()));

            AnchorPane pane = FXMLLoader.load(getClass().getClassLoader().getResource("sample/Completed_Match_Stats_Cricket.fxml"));
...
//        tournament_name1=selectedItem;
    ...
    void see_finished_matches_clicked(ActionEvent event) throws IOException {
        ...
        TournamentMatchScheduleSeeFinishedMatchesCricketController tournamentMatchScheduleSeeFinishedMatchesController=new TournamentMatchScheduleSeeFinishedMatchesCricketController();
